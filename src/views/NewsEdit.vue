  ...
  .newsEdit
    ...
    .wrap
      el-form(ref="form", :model="form", label-width="80px")
        el-row(:gutter="20")
          el-col(:span="14")
            el-divider(content-position="left") 基础设置
            el-form-item(label="标题")
              el-input(v-model="form.title", clearable, maxlength="40", show-word-limit)
            el-form-item(label="简介")
              el-input(v-model="form.brief", clearable, maxlength="100", show-word-limit)
            el-form-item(label="作者")
              el-input(v-model="form.author", clearable, maxlength="10", show-word-limit)
            el-form-item(label="内容")
              Editor(:value="form.content")
          el-col(:span="10")
            el-divider(content-position="left") 高级设置
            el-form-item(label="栏目")
              el-select(v-model="form.columnId", placeholder="请选择栏目")
                el-option(v-for="c in columns", :key="c.id", :label="c.name", :value="c.id")
            el-form-item(label="分类")
              el-select(v-model="form.tagId", placeholder="请选择事件分类")
                el-option(v-for="c in tags", :key="c.id", :label="c.name", :value="c.id")
            el-form-item(label="外链")
              el-input(v-model="form.linkUrl", clearable, maxlength="200", placeholder="填写此项即跳转")
            el-form-item(label="发布时间")
              el-date-picker(v-model="form.insertTime", type="datetime", placeholder="请选择发布日期")
